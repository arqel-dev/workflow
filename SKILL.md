...
**Eventos (WF-004).** `Events\StateTransitioned` (final) é disparado *após* uma transição bem-sucedida. Carrega `record: Model`, `from: string`, `to: string`, `userId: ?int`, `context: array<string,mixed>`. Implementa `Dispatchable` + `SerializesModels`. **Não** implementa `ShouldBroadcast` por design — broadcasting é opt-in via listener dedicado. `HasWorkflow::transitionTo($newState, array $context = [])` captura state atual, delega à API spatie quando o state object atual tem `transitionTo()` (caminho spatie inalterado — mantém os guards dele), ou faz attribute assignment + save direto, e em seguida dispara o evento (capturando `Auth::id()`). **No caminho fallback (não-spatie) o método enforça WF-006 *antes* de mutar/persistir** (issue #84): se o model declara transitions, o `newState` precisa ser alcançável (alguma transition declarada cujo `from()` inclua o state atual — ou aberta/sem `from()` — e cujo destino bata com `newState` via slug canônico), senão lança `Exceptions\IllegalTransitionException`; entre as transitions que casam, ao menos uma precisa ser autorizada pelo `TransitionAuthorizer` (deny-by-default, usuário via `Auth::user()`), senão lança `Exceptions\UnauthorizedTransitionException`. Models **sem** transitions (`transitions([])`, como o `Ticket` da showcase) permanecem **free-form**: nenhuma eligibility/autorização é imposta. Suprime o evento quando `arqel-workflow.audit.enabled === false` ou `audit.log_via !== 'event'` — útil para migrations/seeders. O trait opcional `Concerns\RecordsStateTransition` permite que classes spatie `Transition` despachem o evento canônico do próprio `handle()`.
...
**Histórico append-only (WF-007).** `Models\StateTransition` (final, `$timestamps = false`, `metadata` cast `array`, `created_at` cast `datetime`) persiste cada transição. Migration `2026_05_01_000000_create_arqel_state_transitions_table` (registrada via `hasMigration('2026_05_01_000000_create_arqel_state_transitions_table')` — nome **datado**, casando o ficheiro shipado para `vendor:publish` resolver, #52) cria `arqel_state_transitions` com `morphs('model')`, `from_state` (nullable), `to_state`, `transitioned_by_user_id` (indexed), `metadata` (JSON), `created_at` com `useCurrent()` (sem `updated_at` — append-only). `Listeners\PersistStateTransitionToHistory` (auto-registado pelo provider) escuta `StateTransitioned` e grava — skip silencioso quando `arqel-workflow.history.enabled === false`; captura `Throwable` para não bloquear a transição do domínio. **O `model_type` é gravado via `getMorphClass()`** (não o FQCN cru), de modo que sob um `Relation::enforceMorphMap([...])` a linha leva o alias — alinhado ao que a relação consulta (#164). `HasWorkflow::stateTransitions(): MorphMany` é um `morphMany(StateTransition::class, 'model')` (consulta `model_type = getMorphClass()`) ordenado por `created_at desc, id desc`. `StateTransition::user()` retorna `?BelongsTo` defensivo (lê `arqel-workflow.user_model`, default `App\\Models\\User`; `null` quando ausente). `Fields\StateTransitionField::resolveHistory()` lê o histórico real filtrado por `(model_type, model_id)` — **também chaveando `model_type` por `getMorphClass()`**, consistente com o writer + o `morphMany` — com limit configurável (`arqel-workflow.history.limit`, default 50) — best-effort com `Throwable` capturado.

**Field React-bound (WF-005/006).** `Fields\StateTransitionField` (extends `Arqel\Fields\Field`) serializa `currentState: {name, label, color, icon}|null`, `transitions: list<{from, to, label, authorized}>` (delegando ao `TransitionAuthorizer`) e `history` (vinda de `StateTransition`). **Emite exatamente uma entry por transition aplicável** (espelha `HasWorkflow::getAvailableTransitions`): uma transition cujo `from()` liste vários states — incluindo o atual — gera **um único** botão com `from` = state corrente do record, nunca um botão por from-state declarado (o que produzia botões duplicados carregando um `from` que não era o state atual — #154). Transitions cujo `from()` não inclui o state corrente são filtradas; transitions sem `from()` (any-to-X) sempre aparecem. O token `to` é o **segmento de destino** da transition, resolvido pelo helper compartilhado `Support\TransitionTargetResolver::resolve()` — `to()` estático quando declarado, senão a convenção `XxxToYyy` via `/To([A-Z][A-Za-z0-9]*)$/` (`PendingToPaid` ⇒ `Paid`), senão o short name. **O mesmo resolver alimenta o write-path (`HasWorkflow::resolveTransitionTo()`) e o authorizer (`TransitionAuthorizer::resolveTo()`)**, garantindo que o token anunciado pelo botão round-trippe pelo `assertTransitionAllowed()` (#119) — nunca emita o short name completo da classe. Métodos fluentes: `showDescription()`, `showHistory()`, `transitionsAttribute()`, `record()`. O componente React `arqel-dev/workflow/StateTransition` (slice C29) consome estes props.
...
**Cobertura de testes (WF-009).** 77 testes Pest 3 (Orchestra Testbench + sqlite in-memory): `WorkflowDefinitionTest` (7), `HasWorkflowTest` (6), `StateTransitionedEventTest` (8), `StateTransitionHistoryTest` (7), `TransitionToEnforcementTest` (7, WF-006 fallback enforcement — issue #84), `WorkflowServiceProviderTest` (2), `StateTransitionFieldTest` (~12, incl. round-trip do token `to` — issue #119), `TransitionAuthorizerTest` (10), `StateFilterTest` (12), e o `Unit/Coverage/CoverageGapsTest` (9 cobrindo edge-cases de `guessLabel`, ordem de `getTransitions`, replace vs merge em `states()`, `slugifyState` para wildcard/digits/bare strings, `apply()` com array misto, defaults em `optionsArray`, round-trip de cast `metadata`, MorphTo `model()`, e ordem em `resolveAvailableTransitions`). PHPStan level max clean.
...
- **Usar `getAvailableTransitions()` como autorização** — é metadata para UI. Authorization real fica em `TransitionAuthorizer` e, no fallback de `transitionTo()`, é enforçada server-side (WF-006 / issue #84).
- **Assumir que `transitionTo()` no caminho fallback é livre** — ele agora rejeita transitions ilegais (`IllegalTransitionException`) e não-autorizadas (`UnauthorizedTransitionException`) quando o model declara transitions. Capture-as no controller/action. Models `transitions([])` continuam free-form.
